...
const NODES = [
  ...
  { key: "target", lines: ["Signals &", "Enrichment"], lx: 256, ly: 206, anchor: "middle" as const },
...
export function VirtuousCycle() {
  ...
  return (
    <div className="mt-6">
      ...
      <svg viewBox="0 0 340 300" className="mt-2 w-full" role="img" aria-label="AI and tech virtuous cycle">
        {/* spokes: the engine powers every stage */}
        {NODES.map((n) => (
          <line key={`s-${n.key}`} x1={C.x} y1={C.y} x2={n.x} y2={n.y} className="stroke-foreground/10" />
        ))}
        ...
        {!reduced && (
          ...
          >
            <circle cx={C.x} cy={C.y - R} r={9} className="fill-haveli-gold/20" />
            <circle cx={C.x} cy={C.y - R} r={3.5} className="fill-haveli-gold" />
          </g>
        )}

        {/* center hub */}
        <circle cx={C.x} cy={C.y} r={28} className="fill-card stroke-haveli-gold/50" />
        <text x={C.x} y={C.y - 1} textAnchor="middle" className="fill-haveli-gold font-serif" fontSize="16">
          AI
        </text>
        <text
          x={C.x}
          ...
          return (
            <g key={n.key}>
              <circle
                ...
                className={isActive ? "fill-haveli-gold" : "fill-card stroke-haveli-gold/40"}
              />
              {n.lines.map((line, li) => (
                <text
                  key={li}
      ...
      <div className="mt-1 space-y-2 border-t border-foreground/10 pt-4">
        <p className="text-sm leading-snug text-foreground/90">
          <span className="text-haveli-gold">3× leverage</span> for every investment team member.
        </p>
